...
import {
  getComponentOptions, getOptionsOfEnum, getTypeFromUnion, getTypeInfo, humanize, isArraysShallowDiffers, merge, move,
  UIDGenerator
} from "./util";
...
export class Select extends Component {
  static transformer = (nullOption) => {
    return {
      format: (value) =>
        Nil.is(value) && nullOption ? nullOption.value : value,
      parse: (value) =>
        nullOption && nullOption.value === value ? null : value,
    };
  };
...
export class Form extends React.Component {
  ...
  validateFields(fields) {
    return fields.map((field) => {
      const points = t.String.is(field) ? field.split(".") : field;
      const element = points.reduce(
        (input, name) => input.childRefs[name],
        this.inputRef
      );
